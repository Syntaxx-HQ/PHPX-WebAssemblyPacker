...
(() => {
    ...
    function fetchRemotePackage(packageName, packageSize, callback, errback) {
      ...
      xhr.onprogress = function (event) {
        ...
        if (event.loaded) {
          if (!xhr.addedTotal) {
            xhr.addedTotal = true;
            if (!createPhpModule.dataFileDownloads)
              createPhpModule.dataFileDownloads = {};
          ...
          total = Math.ceil(
            (total * createPhpModule.expectedDataFileDownloads) / num,
          );
          if (createPhpModule.setStatus)
            createPhpModule.setStatus(
              "Downloading data... (" + loaded + "/" + total + ")",
            );
        } else if (!total) {
          if (createPhpModule.setStatus)
            createPhpModule.setStatus("Downloading data...");
        }
      };
      ...
      `;
        if (metadata["LZ4"]) {
          if (typeof LZ4 === "undefined") {
            console.error(
              "LZ4 decoder not found. Make sure lz4.js is included.",
            );
            throw new Error("LZ4 decoder missing");
          }
          console.log(
            "Decompressing " + byteArray.length + " bytes of LZ4 data",
          );
          try {
            ...
            throw new Error(
              "Failed to decompress LZ4 data: " + (e.message || e),
            );
          }
        }
  ...
  loadPackage({
    files: [
      { filename: "/test_dir/include.txt", start: 0, end: 13, audio: 0 },
      {
        filename: "/test_dir/subdir/another.txt",
        start: 13,
        end: 26,
        audio: 0,
      },
